...
func StackExecCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: `Execute a script in stack instance(s). You can use the --target
flag more than once.

Examples:

  $ ` + cmdline.Args0() + ` stack exec --target "stackid|ip1,ip2" --script /path/to/script`,
		Run: stackExec,
	}

	cmd.Flags().SortFlags = false
	cmd.Flags().StringSliceVar(&targets, "target", targets, "`fmt`: stackid|ip1,ip2,...")
